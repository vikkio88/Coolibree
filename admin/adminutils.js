...
//		var left=((($(window).width()/ 2)+($(window).scrollLeft())) + "px");
		...
		data='<form action="javascript:'+action+'($(\'#pass\').val());" onsubmit="'+action+'($(\'#pass\').val());" style="text-align: center;padding=5px;"><h3>Reinsert Password</h3><input type="password" id="pass" /><br />'
...
function makepost(passw){
	...
	//alert(passw);
	$.post(url, { u: us, p: passw, title: tit, message: post},
   function(data){
...
function makeeditpost(passw){
	...
	//alert(idval);
	$.post(url, { u: us, p: passw, body: postv, id: idval},
   function(data){
	  closenotification();
      $("#target").hide("slow",function(){$("#target").html("");$.notifyBar({ cls: "success", html: data});})
   });
...
function makedelpost(passw){
	...
	//alert(passw);
	$.post(url, { u: us, p: passw, body: postv, pos: idval},
   function(data){
	  closenotification();
      $("#target").hide("slow",function(){$("#target").html("");$.notifyBar({ cls: "success", html: data});})
   });	
...
function view_logs(passw){
	...
	if(passw=="")
		...
		endiv='</div>';
		$.post(url,{ u: us, p: passw},function(data){
...
function makeedittemp(passw){
		...
		$.post(url,{u: us, p: passw, style: css, bar: navibar, header: head, footer: foot},function(data){
							$("#target").hide("slow",function(){$("#target").html("");$.notifyBar({ cls: "success", html: data});});
						}
		);
}
